...
def test_initialize_with_feedback_command():
    """Test initialize with feedback command"""
    mock_command = Mock(return_value=0)

    with (
        patch("sys.argv", ["c", "feedback"]),
        patch("command_line_assistant.utils.cli.register_all_commands"),
        patch("command_line_assistant.client.read_stdin", lambda: None),
        patch("argparse.ArgumentParser.parse_args") as mock_parse,
    ):
...
def test_command_returns_non_zero_exit_code():
    """Test that commands can return non-zero exit codes"""
    mock_command = Mock(return_value=1)

    with (
        patch("sys.argv", ["c", "shell"]),
        patch("command_line_assistant.utils.cli.register_all_commands"),
        patch("command_line_assistant.client.read_stdin", lambda: None),
        patch("argparse.ArgumentParser.parse_args") as mock_parse,
    ):
