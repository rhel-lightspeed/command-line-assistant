...
class Animation:
    ...
    Attributes::
        frames (Iterator[str]): An infinite iterable of the animations frames.  You can use
            :func:`itertools.cycle` to make aany iterable into an appropriate infinite iterable.
        encoding str: The name of the encoding of the characters in the frames.  This should
            generally be `ascii` if the characters are all present in the ascii characterset or
            `utf8` if they are restricted to characters in the unicode standard but other encodings
            can be used if needed.  Note that this is merely an encoding that the characters **can**
            be encoded to.  The actual encoding in this file is utf-8.  This defaults to `utf8`.
    """
